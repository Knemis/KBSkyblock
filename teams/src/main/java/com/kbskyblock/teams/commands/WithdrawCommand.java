...
@NoArgsConstructor
public class WithdrawCommand<T extends Team, U extends KBSkyblockUser<T>> extends Command<T, U> {
    ...
    public boolean execute(U user, T team, String[] args, KBSkyblockTeams<T, U> teams) {
        ...
        try {
            ...
            BankResponse bankResponse = bankItem.get().withdraw(player, Double.parseDouble(args[1]), teamBank, teams);

            player.sendMessage(StringUtils.color((bankResponse.isSuccess() ? teams.getMessages().bankWithdrew : teams.getMessages().insufficientFundsToWithdraw)
