...
@NoArgsConstructor
public class ChatCommand<T extends Team, U extends KBSkyblockUser<T>> extends Command<T, U> {
    ...
    public boolean execute(U user, T team, String[] args, KBSkyblockTeams<T, U> teams) {
        ...
        if (!chatType.isPresent()) {
            player.sendMessage(StringUtils.color(teams.getMessages().unknownChatType
        ...
        player.sendMessage(StringUtils.color(teams.getMessages().setChatType
                .replace("%prefix%", teams.getConfiguration().prefix)
                .replace("%type%", chat))
        );
        return true;
    ...
    public List<String> onTabComplete(CommandSender commandSender, String[] args, KBSkyblockTeams<T, U> teams) {
        return teams.getChatTypes().stream()
