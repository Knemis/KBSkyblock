...
@NoArgsConstructor
public class InviteCommand<T extends Team, U extends KBSkyblockUser<T>> extends Command<T, U> {
    ...
    public boolean execute(U user, T team, String[] args, KBSkyblockTeams<T, U> teams) {
        ...
        if (!teams.getTeamManager().getTeamPermission(team, user, PermissionType.INVITE)) {
            player.sendMessage(StringUtils.color(teams.getMessages().cannotInvite
        ...
        if (teams.getTeamManager().getTeamInvite(team, offlinePlayerUser).isPresent()) {
            player.sendMessage(StringUtils.color(teams.getMessages().inviteAlreadyPresent
                    .replace("%prefix%", teams.getConfiguration().prefix)
            ));
            return false;
        }

        teams.getTeamManager().createTeamInvite(team, offlinePlayerUser, user);
        player.sendMessage(StringUtils.color(teams.getMessages().teamInviteSent
                .replace("%prefix%", teams.getConfiguration().prefix)
                .replace("%player%", offlinePlayerUser.getName())
        ));
        invitee.sendMessage(StringUtils.color(teams.getMessages().teamInviteReceived
                .replace("%prefix%", teams.getConfiguration().prefix)
