...
@NoArgsConstructor
public class InfoCommand<T extends Team, U extends KBSkyblockUser<T>> extends Command<T, U> {
    ...
    public boolean execute(U user, String[] args, KBSkyblockTeams<T, U> teams) {
        ...
        if (args.length == 0) {
            ...
            sendTeamInfo(player, userTeam.get(), teams);
            return true;
        }
        ...
        sendTeamInfo(player, team.get(), teams);
        return true;
    }

    public void sendTeamInfo(Player player, T team, KBSkyblockTeams<T, U> teams) {
        ...
        for (String line : teams.getConfiguration().teamInfo) {
