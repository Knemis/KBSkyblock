...
import com.kbskyblock.teams.enhancements.FarmingEnhancementData;
...
@AllArgsConstructor
public class BlockGrowListener<T extends Team, U extends KBSkyblockUser<T>> implements Listener {
        ...
        teams.getTeamManager().getTeamViaLocation(event.getBlock().getLocation()).ifPresent(team -> {
            if (event.getNewState().getBlockData() instanceof Ageable) {
                ...
                if (ageable.getAge() >= ageable.getMaximumAge() || ageIgnoreList.contains(material)) {
                    teams.getMissionManager().handleMissionUpdate(team, event.getBlock().getLocation().getWorld(), "GROW", material.name(), 1);
                }

                Enhancement<FarmingEnhancementData> farmingEnhancement = teams.getEnhancements().farmingEnhancement;
                TeamEnhancement teamEnhancement = teams.getTeamManager().getTeamEnhancement(team, "farming");
