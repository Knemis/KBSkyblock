...
public class BoostersGUI<T extends Team, U extends KBSkyblockUser<T>> extends BackGUI {

    private final T team;
    private final KBSkyblockTeams<T, U> teams;
    private final Map<Integer, String> boosters = new HashMap<>();

    public BoostersGUI(T team, Player player, KBSkyblockTeams<T, U> teams) {
        super(teams.getInventories().boostersGUI.background, player, teams.getInventories().backButton);
        this.team = team;
        this.teams = teams;
    }
    ...
    public Inventory getInventory() {
        NoItemGUI noItemGUI = teams.getInventories().boostersGUI;
    ...
    public void addContent(Inventory inventory) {
        ...
        for (Map.Entry<String, Enhancement<?>> enhancementEntry : teams.getEnhancementList().entrySet()) {
            ...
            String nextLevel = nextData == null ? teams.getMessages().nullPlaceholder : String.valueOf(currentLevel + 1);
            String cost = nextData == null ? currentData == null ? teams.getMessages().nullPlaceholder : String.valueOf(currentData.money) : String.valueOf(nextData.money);
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        teams.getCommandManager().executeCommand(event.getWhoClicked(), teams.getCommands().boostersCommand, new String[]{"buy", booster});
    }
