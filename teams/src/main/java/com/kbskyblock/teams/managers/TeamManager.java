...
public abstract class TeamManager<T extends Team, U extends KBSkyblockUser<T>> {
    ...
    public TeamManager(KBSkyblockTeams<T, U> teams) {
        this.teams = teams;
    }
    ...
    public List<T> getTeams(TeamSorting<T> sortType, boolean excludePrivate) {
        return teamTopCache.get(sortType, Duration.ofSeconds(10), () -> sortType.getSortedTeams(teams).stream()
                .filter(team -> {
    ...
    public List<T> getTeams(SortType sortType, boolean excludePrivate) {
        switch (sortType) {
            case Value:
                return getTeams(teams.getTop().valueTeamSort, excludePrivate);
            case Experience:
                return getTeams(teams.getTop().experienceTeamSort, excludePrivate);
            default:
                return getTeams();
        }
    }
    ...
    public List<U> getTeamMembers(T team) {
        return teams.getUserManager().getUsers().stream()
    ...
    public int getTeamLevel(int experience) {
        if (!teams.getConfiguration().enableLeveling) return 1;

        int flatExpRequirement = teams.getConfiguration().flatExpRequirement;
        double curvedExpModifier = teams.getConfiguration().curvedExpModifier;
    ...
    public double getTeamValue(T team) {
        return teamValueCache.get(team, Duration.ofSeconds(1), () -> {
            ...
            for (Map.Entry<XMaterial, BlockValues.ValuableBlock> valuableBlock : teams.getBlockValues().blockValues.entrySet()) {
                value += getTeamBlock(team, valuableBlock.getKey()).getAmount() * valuableBlock.getValue().value;
            }

            for (Map.Entry<EntityType, BlockValues.ValuableBlock> valuableSpawner : teams.getBlockValues().spawnerValues.entrySet()) {
    ...
    public boolean UpdateEnhancement(T team, String booster, Player player) {
        ...
        if (teams.getEconomy().getBalance(player) < enhancementData.money) {
            player.sendMessage(StringUtils.color(teams.getMessages().notEnoughMoney
                    .replace("%prefix%", teams.getConfiguration().prefix)
            ));
            return false;
        }
        ...
        for (Map.Entry<String, Double> bankCost : enhancementData.bankCosts.entrySet()) {
            if (getTeamBank(team, bankCost.getKey()).getNumber() < bankCost.getValue()) {
                player.sendMessage(StringUtils.color(teams.getMessages().notEnoughBankItem
                        .replace("%prefix%", teams.getConfiguration().prefix)
        ...
        teams.getEconomy().withdrawPlayer(player, enhancementData.money);
    ...
    public List<String> getTeamMission(T team, MissionType missionType) {
        ...
        List<TeamMission> teamMissions = getTeamMissions(team).stream()
                .filter(teamMission -> teams.getMissions().missions.containsKey(teamMission.getMissionName()))
                .filter(teamMission -> teams.getMissions().missions.get(teamMission.getMissionName()).getMissionType() == missionType)
                .filter(teamMission -> teams.getMissions().missions.get(teamMission.getMissionName()).getMissionData().get(teamMission.getMissionLevel()).getItem().slot == null)
                .collect(Collectors.toList());
        ...
        List<Map.Entry<String, Mission>> availableMissions = teams.getMissions().missions.entrySet().stream()
                ...
                .collect(Collectors.toList());
        while (missions.size() < teams.getMissions().dailySlots.size() && availableMissions.size() > 0) {
    ...
    public synchronized void claimTeamReward(TeamReward teamReward, Player player) {
        ...
        teams.getEconomy().depositPlayer(player, reward.money);
    ...
    public boolean isBankItem(ItemStack item) {
        ...
        NBT.get(item, readableItemNBT -> {
            return readableItemNBT.hasTag(teams.getName().toLowerCase(), NBTType.NBTTagCompound);
        });
